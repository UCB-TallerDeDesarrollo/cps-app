...
.factory('DataSeed', function() {
  ...
  function seedChildsConcerns($cordovaSQLite, db){
    $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS childs_concerns (id integer primary key autoincrement, description text, unsolved_problem_id integer,  sort_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP , FOREIGN KEY (unsolved_problem_id) REFERENCES unsolved_problems (id))");
  }
